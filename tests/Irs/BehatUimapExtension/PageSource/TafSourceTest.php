...
class TafTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAllTafPagesShouldBeLoaded()
    {
        $path = __DIR__ . '/taf_uimaps';
        $source = new TafSource($path);

        foreach ($this->_allPageKeys as $key) {
            try {
                $source->getPageByKey($key);
            } catch (OutOfRangeException $e) {
                $this->fail("Page $key does not exist.");
            }
        }
    }
